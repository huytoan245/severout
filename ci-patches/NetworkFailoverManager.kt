...
/**
 * Keeps Wi-Fi connected for the phone, but can temporarily bind only this app
 * process to a Cellular path when Wi-Fi cannot reach Firebase.
 */
class NetworkFailoverManager(
    ...
    fun probeNow() {
        if (!stopped.get()) main.post { evaluate() }
    }
    ...
    private fun requestCellular() {
        ...
        val callback = object : ConnectivityManager.NetworkCallback() {
            override fun onAvailable(network: Network) {
                ...
                probe(network) { ok ->
                    if (!ok || stopped.get() || hasActiveVpn()) return@probe
                    if (stopped.get() || failoverActive) return@probe
                    val bound = try { cm.bindProcessToNetwork(network) } catch (_: Exception) { false }
                    if (bound) {
                        failoverActive = true
                        failoverSince = System.currentTimeMillis()
                        wifiRecoverySuccesses = 0
                        updateMode("cellular_failover", forceNotify = true)
                    }
                }
            }
